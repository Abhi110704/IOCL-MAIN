...
router.post('/', authenticate, authorize(['ADMIN', 'MENTOR']), asyncHandler(async (req, res) => {
...
router.get('/', authenticate, asyncHandler(async (req, res) => {
    const { page = 1, limit = 10, status, priority, mentorId, internId } = req.query;
...
router.delete('/:id', authenticate, authorize(['ADMIN', 'MENTOR']),
  asyncHandler(async (req, res) => {
    const { id } = req.params;
